        ...
        <div class="col-md-10 col-sm-12 col-centered">    
          <div class="checkout-content">
            ...
            <div id="cart_summary" class="step well">
              <h2 class="step-title">Sommaire de vos achats</h2><hr>
              <div class="shopping-cart-summary-content">
                <ul class="cart-data">
                  <li class="row list-inline columnCaptions">
                    <div class="header-items">Produit </div>
                    <div class="header-price">Prix </div>
                    <div class="header-qty">Quantitée</div>
                  ...
                  @foreach(Cart::items() as $index => $items)
                    <li class="row items-inline">
                        ...
                        @if($items->img_src)
                          <div class="product-img">
                            <a href="{{ route('details-page', get_product_slug($items->id)) }}">
                              <img src="{{ $items->img_src }}" alt="product">
                            </a>
                          </div>
                        @else
                          <div class="product-img">
                            <a href="{{ route('details-page', get_product_slug($items->id)) }}">
                              <img src="{{ url('/') }}/public/images/no-image.png" alt="product">
                            </a>
                          </div>
                        @endif
                        <div class="item-name">
                          ...
                          @if(get_product_type($items->id) === 'customizable_product')
                            @if($items->acces_token)
                              @if(count(get_customize_images_by_access_token($items->acces_token))>0)
                                <button class="btn btn-block btn-xs view-customize-images" data-images="{{ htmlspecialchars(json_encode( get_customize_images_by_access_token($items->acces_token) )) }}">{{ trans('frontend.design_images') }}</button>
                              @endif
                            @endif
                          @endif
                        </div>
                      ...
                      <div class="price line-total">{!! price_html(  get_product_price_html_by_filter(Cart::getRowPrice($items->quantity, $items->price) )) !!}</div>
                  ...
                  <li class="row cart-button-main">
                      <div class="apply-coupon">
                          <input type="text" class="form-control" id="apply_coupon_code" name="apply_coupon" placeholder="Entrez un couppon">
                          <button class="btn btn-primary" name="apply_coupon_post" id="apply_coupon_post">Appliquer le coupon</button>
                      </div>
                    <div class="btn-cart-action">
                      <button class="btn btn-primary empty" type="submit" name="empty_cart" value="empty_cart">Vider le panier</button>
                      <button class="btn btn-primary update" type="submit" name="update_cart" value="update_cart">Mettre à jour le panier</button>
                    </div>
                  </li>
            ...
            <div id="user_mode" class="step well">
              <h2 class="step-title">Mode d'utilisateur</h2><hr>  
              <div class="checkout-process-user-mode">
                <ul class="nav">
                  <li>
                      <label><input type="radio" class="shopist-iCheck" name="user_checkout_complete_type" value="guest">&nbsp; Invité</label>
                  </li>
                  <li>
                    <label><input type="radio" class="shopist-iCheck" name="user_checkout_complete_type" value="login_user">&nbsp; Se connecter </label>
                  </li>
                </ul>
              </div>
            </div>
            ...
            <div id="authentication" class="step well">
              <h2 class="step-title">S'enregistrer</h2><hr>  
              <div class="user-login-content">
                <div class="login-information clearfix">
                  <div class="login-content-sub">
                    <h3 class="page-subheading">Créer un compte</h3>
                    <div class="form_content">
                      <p>Si vous n'avez pas de compte, veuillez s'il vous plait en créer un</p>
                      <a class="btn btn-primary" href="{{ route('user-registration-page') }}">Créer un compte</a>
                    </div>
                  </div>
                  <div class="login-content-sub">
                    <h3 class="page-subheading">Vous avez déja un compte ?</h3>
                    <div class="form_content">
                      <p>Si vous avez déja un compte, veuillez vous connecter</p>
                      <a class="btn btn-primary" href="{{ route('user-login-page') }}">Se connecter</a>
                    </div>
                  </div>
                </div>  
              </div>
            ...
            <div id="login_user_address" class="step well">
              ...
              <div class="text-right">
                @if(!empty($login_user_account_data) && !empty($login_user_account_data->address_details))
                  <a href="{{ route('my-address-edit-page') }}" class="btn btn-primary btn-sm">Modifier l'adresse</a>
                @else
                  <a href="{{ route('my-address-add-page') }}" class="btn btn-primary btn-sm">Ajouter une addresse</a>
                @endif
              </div>
              <br>
              <div class="user-address-content">
                <div class="address-information clearfix">
                  <div class="address-content-sub">
                    <h3 class="page-subheading">Adresse de facturation</h3><br>
                    @if(!empty($login_user_account_data) && !empty($login_user_account_data->address_details))
                  ...
                  <div class="address-content-sub">
                    <h3 class="page-subheading">Adresse de livraison</h3><br>
            ...
            <div id="payment" class="step well">
              <h2 class="step-title">Choisissez une méthode de payment</h2><hr>
              @if($payment_method_data['payment_option']['enable_payment_method'] == 'yes' && ($payment_method_data['bacs']['enable_option'] == 'yes' || $payment_method_data['cod']['enable_option'] == 'yes' || $payment_method_data['paypal']['enable_option'] == 'yes' || $payment_method_data['stripe']['enable_option'] == 'yes'))
                <div class="payment-options">
                  ...
                  <span>
                   <label>
                     @if(old('payment_option') == 'cod')
                      <input type="radio" checked name="payment_option" class="shopist-iCheck" value="cod"> Payment à la livraison
                     @else
                      <input type="radio" name="payment_option" class="shopist-iCheck" value="cod"> Payment à la livraison
                     @endif
                   </label>
                  </span>
                 @endif
                 ...
                 @if($payment_method_data['stripe']['enable_option'] == 'yes')
                  ...
                  <input type="hidden" name="stripe_api_key" value="{{ htmlspecialchars($stripe_api_key) }}" id="stripe_api_key">
                  <span>
                   <label>
            ...
            <div id="order_notes" class="step well">
              <h2 class="step-title">Ajouter un commentaire (optionnel)</h2><hr>
              <div class="order-extra-notes">
                <textarea name="checkout_order_extra_message" id="checkout_order_extra_message"  placeholder="Notes à propos de vos achats" class="form-control">{!! old('checkout_order_extra_message') !!}</textarea>
              </div>
            </div>
            @endif
            
            <button class="action next btn btn-primary">Continuer</button>
            <button name="checkout_proceed" class="action submit btn btn-primary place-order" type="submit" value="checkout_proceed">Commander</button>
          </div>
        </div>  
