...
class AdminDashboardController extends Controller
{
  ...
  public function redirectToAdminDashboard($params = null){
    ...
    if(Request::is('admin/blog/comments-list')){
      ...
      $this->dashboardData['orders_list_data']   =   $order_object;
        ...
        $order_data_by_id['_talon_image'] = $get_post_by_order_id->talon_image;
        $order_data_by_id['_ship_town'] = $get_post_by_order_id->ship_town;
        ...
        foreach($lastProducts as $rows){
          ...
          $products_data['img_url'] = URL::to('/public/images/no-image.png');
          }
  ...
  public function updateOrderStatus($order_id)
  {
    if( Request::isMethod('post') && Session::token() == Input::get('_token'))
    {
      ...
      {
        $order_statue = PostExtra::where(['post_id' => $order_id, 'key_name' => '_order_status'])->first()->key_value;
        if(strcmp( $order_statue, 'completed') == 0)
        {
        ...
        $user_id = Post::where('id', $order_id)->first()->post_author_id;
        $message = Input::get('message_status');
        $notifications = new Notification;
        $notifications->order_id = $order_id;
        $notifications->user_id = $user_id;
        $notifications->message = $message;
        if($order_statue == 'cancelled')
        {
          $notifications->order_statue = 0;
        } else {
          $notifications->order_statue = 1;
        }
        $notifications->save();
