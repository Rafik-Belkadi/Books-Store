...
class FrontendManagerController extends Controller
{
  ...
  public function forceDownload( $post_id, $order_id, $file_key, $target ){
    $get_product_data = $this->product->getProductDataById( $post_id );
    ...
    if(isset($get_product_data['post_type']) && $get_product_data['post_type']== 'product_variation'){
      $file_key_name = '_downloadable_product_data';
    }
    else{
      $file_key_name = '_downloadable_product_files';
    }
    
    if( $this->classCommonFunction->checkDownloadRequired( $post_id, $order_id ) && isset($get_product_data[$file_key_name][$file_key]) && isset($get_product_data[$file_key_name][$file_key][$target]) ){
      $parse_url = explode('uploads', $get_product_data[$file_key_name][$file_key][$target]);
      ...
      if(count($parse_url) > 0 && isset($parse_url[1])){
        ...
        if(File::exists($file_path)){
          ...
          if(!empty($get_extension) && !empty($get_content_type)){
            ...
            $downloadextra->file_name		 =   $get_product_data[$file_key_name][$file_key]['file_name'];
  ...
  public function addTalonImage($order_id){

    

    $post = Post::find($order_id);
    $destinationPath = public_path().'/uploads/talons/';
    if (Input::hasFile('talon_image'))
    {
      // Get FileName with Extension
      $fileNameWithExt = Input::file('talon_image')->getClientOriginalName();
      
      //Get just FileName
      $filename = 'Image_Talon_Commande_Num_'.

      // Get just Ext
      $extension = Input::file('talon_image')->getClientOriginalExtension();
      
      // Filename To store
      $fileNameToStore = $filename.'_'.$order_id.'.'.$extension;

      Input::file('talon_image')->move($destinationPath, $fileNameToStore);
      $town = Input::get('change_town');

      $post->ship_town = $town;
      $post->talon_image = $fileNameToStore;
      $post->save();
