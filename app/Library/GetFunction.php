...
use shopist\Models\ManufacturersList;
use shopist\Models\TagsList;
...
class GetFunction
{
  ...
  {
    $new_ary = array();
    $method  = array('bacs' => Lang::get('admin.direct_bank_transfer'), 'cod' => Lang::get('admin.cash_on_delivery'), 'paypal' => Lang::get('admin.paypal'));
  ...
  public function sendCustomMail($data)
  {
    ...
    if($data['source'] == 'order_confirmation')
    {
      ...
      $get_view_data['_subject']    = Lang::get('admin.mail_subject') . ' '.$this->carbonObject->parse( $this->carbonObject->today() )->format('F d, Y');
    }
    ...
    {
      Mail::send($view, $get_view_data, function ($message) use ($get_view_data)
      {
        $message->from($get_view_data['_from_email'], $get_view_data['_site_title']);
        $message->to($get_view_data['_mail_to'])->subject($get_view_data['_subject']);
      });
    }
  }
  ...
  public static function product_img($product_id){
    $image_data = '';
    $get_post_meta    =   PostExtra :: where(['post_id' => $product_id, 'key_name' => '_product_related_images_url'])->first();
    
    if(!empty($get_post_meta)){
      $image_data = json_decode($get_post_meta['key_value'])->product_image;
    }
  ...
  public static function product_price($product_id){ 
    $final_price = 0;
    $_this            =   new self;
    $get_post_meta    =   PostExtra :: where(['post_id' => $product_id, 'key_name' => '_product_price'])->first();
    $final_price      =   $get_post_meta['key_value'];
  ...
  public static function product_type( $product_id = '' ){
    $product_type =  '';
    $post_meta    =  PostExtra::where(['post_id' => $product_id, 'key_name' => '_product_type'])->first();
    
    if(!empty($post_meta->key_value)){
      $product_type = $post_meta->key_value;
    }
  ...
  public static function admin_customize_images_by_access_token( $product_id, $order_id, $access_token )
  {
    ...
    {
      $get_images = UsersCustomDesign::where(['product_id' => $product_id, 'order_id' => $order_id, 'access_token' => $access_token])->lists('design_images');
      
      if($get_images->count() > 0)
      {
        $images = unserialize($get_images[0]);
      }
    }
  ...
  public static function check_design_enable_by_product_id($product_id){
    if($product_id > 0){
      $post_meta   =   PostExtra::where(['post_id' => $product_id, 'key_name' => '_product_enable_as_custom_design'])->first();
  ...
  public static function product_review($product_id)
  {
    if($product_id)
    {
      ...
      $post_meta_1   =   PostExtra::where(['post_id' => $product_id, 'key_name' => '_product_enable_reviews'])->first();
      $post_meta_2   =   PostExtra::where(['post_id' => $product_id, 'key_name' => '_product_enable_reviews_add_link_to_product_page'])->first();
      $post_meta_3   =   PostExtra::where(['post_id' => $product_id, 'key_name' => '_product_enable_reviews_add_link_to_details_page'])->first();
      $post_meta_4   =   PostExtra::where(['post_id' => $product_id, 'key_name' => '_product_enable_reviews_totals_with_link'])->first();
  ...
  public static function product_availability( $product_id ){
    ...
    $post_meta  =  PostExtra::where(['post_id' => $product_id, 'key_name' => '_product_manage_stock_availability'])->first();
    
    if(!empty($post_meta)){
      if($post_meta->key_value == 'in_stock'){
  ...
  public static function product_price_by_filter( $amount ){
    ...
    if(Cookie::has('shopist_multi_currency')){
      ...
      if(count($finalData) > 0 && isset($finalData[1])){
        $price =  preg_replace("/[^0-9,.]/", "", $finalData[1]);
      }
      else{
        $price = $amount;
      }
  ...
  public static function permissions_files_list(){
    ...
    $get_permissions_data  =  Option :: where('option_name', '_permissions_files_list')->first();
    $permissions_data      =  unserialize($get_permissions_data->option_value);
  ...
  public static function site_logo_image(){
    ...
    $uploads_url = url('/').'/public/uploads/';
    $image_url   = url('/').'/resources/assets/images/';
    
    if(isset($settings['general_settings']['general_options']['site_logo']) && $settings['general_settings']['general_options']['site_logo']){
      $url = $uploads_url.$settings['general_settings']['general_options']['site_logo'];
    }
    else{
      ...
      if(isset($settings_data['home_details']['cat_list_to_display']) && count($settings_data['home_details']['cat_list_to_display']) > 0){
        ...
        foreach ($settings_data['home_details']['cat_list_to_display'] as $cat){
          ...
          if(!empty($get_parent_categories) && count($get_parent_categories) > 0){
            $get_products_data  =  DB::table('posts')
                                   -> where(['posts.post_status' => 1, 'posts.post_type' => 'product', 'object_relationships.term_id' => $cat, 'post_extras.key_name' => '_product_enable_as_selected_cat', 'post_extras.key_value' => 'yes'])
  ...
  public static function reviews_settings_data($product_id){
    $details = array();
    $get_post_meta_1 = PostExtra::where(['post_id' => $product_id, 'key_name' => '_product_enable_reviews'])->first();
    $get_post_meta_2 = PostExtra::where(['post_id' => $product_id, 'key_name' => '_product_enable_reviews_add_link_to_product_page'])->first();
    $get_post_meta_3 = PostExtra::where(['post_id' => $product_id, 'key_name' => '_product_enable_reviews_add_link_to_details_page'])->first();
  ...
  public static function check_sufficient_permission(){
    ...
    if(count($currentAdminUserData) > 0){
      ...
      if(Request::is('admin/blog/list') && !in_array('all_blogs', $get_role_data->permissions)){
        return false;
      }
      elseif((Request::is('admin/blog/add') || Request::is('admin/blog/update/*')) && !in_array('add_edit_blog', $get_role_data->permissions)){
        return false;
      }
      elseif(Request::is('admin/blog/categories/list') && !in_array('blog_categories', $get_role_data->permissions)){
        return false;
      }
      elseif(Request::is('admin/blog/comments-list') && !in_array('all_blog_comments', $get_role_data->permissions)){
        return false;
      }
      elseif(Request::is('admin/testimonial/list') && !in_array('all_testimonial', $get_role_data->permissions)){
        return false;
      }
      elseif((Request::is('admin/testimonial/add') || Request::is('admin/testimonial/update/*')) && !in_array('add_edit_testimonial', $get_role_data->permissions)){
        return false;
      }
      elseif(Request::is('admin/product/list') && !in_array('all_products', $get_role_data->permissions)){
        return false;
      }
      elseif((Request::is('admin/product/add') || Request::is('admin/product/update/*')) && !in_array('add_edit_product', $get_role_data->permissions)){
        return false;
      }
      elseif(Request::is('admin/product/categories/list') && !in_array('product_categories', $get_role_data->permissions)){
        return false;
      }
      elseif(Request::is('admin/product/tags/list') && !in_array('product_tags', $get_role_data->permissions)){
        return false;
      }
      elseif((Request::is('admin/product/attributes/list') || Request::is('admin/product/attribute/add') || Request::is('admin/product/attribute/update/*')) && !in_array('product_attributes', $get_role_data->permissions)){
        return false;
      }
      elseif(Request::is('admin/product/comments-list') && !in_array('manage_products_comments', $get_role_data->permissions)){
        return false;
      }
      elseif(Request::is('admin/shipping-method/options') && !in_array('manage_shipping_method', $get_role_data->permissions)){
        return false;
      }
      elseif(Request::is('admin/shipping-method/flat-rate') && !in_array('manage_shipping_method', $get_role_data->permissions)){
        return false;
      }
      elseif(Request::is('admin/shipping-method/free-shipping') && !in_array('manage_shipping_method', $get_role_data->permissions)){
        return false;
      }
      elseif(Request::is('admin/shipping-method/local-delivery') && !in_array('manage_shipping_method', $get_role_data->permissions)){
        return false;
      }
      elseif(Request::is('admin/payment-method/options') && !in_array('manage_payment_method', $get_role_data->permissions)){
        return false;
      }
      elseif(Request::is('admin/payment-method/direct-bank') && !in_array('manage_payment_method', $get_role_data->permissions)){
        return false;
      }
      elseif(Request::is('admin/payment-method/cash-on-delivery') && !in_array('manage_payment_method', $get_role_data->permissions)){
        return false;
      }
      elseif(Request::is('admin/payment-method/paypal') && !in_array('manage_payment_method', $get_role_data->permissions)){
        return false;
      }
      elseif(Request::is('admin/manufacturers/list') && !in_array('manage_brands', $get_role_data->permissions)){
        return false;
      }
      elseif((Request::is('admin/manufacturers/add') || Request::is('admin/manufacturers/update/*')) && !in_array('manage_brands', $get_role_data->permissions)){
        return false;
      }
      elseif((Request::is('admin/designer/clipart/categories/list') || Request::is('admin/designer/clipart/category/add') || Request::is('admin/designer/clipart/category/update/*') || Request::is('admin/designer/clipart/list') || Request::is('admin/designer/clipart/add') || Request::is('admin/designer/clipart/update/*') || Request::is('admin/designer/settings')) && !in_array('manage_designer_elements', $get_role_data->permissions)){
        return false;
      }
      elseif((Request::is('admin/orders') || Request::is('admin/orders/details/*') || Request::is('admin/orders/current-date')) && !in_array('manage_orders', $get_role_data->permissions)){
        return false;
      }
      elseif((Request::is('admin/reports') || Request::is('admin/reports/sales-by-product-title') || Request::is('admin/reports/sales-by-month') || Request::is('admin/reports/sales-by-last-7-days') || Request::is('admin/reports/sales-by-custom-days') || Request::is('admin/reports/sales-by-payment-method')) && !in_array('manage_reports', $get_role_data->permissions)){
        return false;
      }
      elseif((Request::is('admin/coupon-manager/coupon/add') || Request::is('admin/coupon-manager/coupon/update/*') || Request::is('admin/coupon-manager/coupon/list')) && !in_array('manage_coupon', $get_role_data->permissions)){
        return false;
      }
      elseif(Request::is('admin/manage/seo') && !in_array('manage_seo', $get_role_data->permissions)){
        return false;
      }
      elseif(Request::is('admin/customer/request-product') && !in_array('manage_requested_product', $get_role_data->permissions)){
        return false;
      }
      elseif((Request::is('admin/subscription/custom') || Request::is('admin/subscription/mailchimp') || Request::is('admin/subscription/settings')) && !in_array('manage_subscription', $get_role_data->permissions)){
        return false;
      }
      elseif((Request::is('admin/settings/general') || Request::is('admin/settings/languages') || Request::is('admin/settings/languages/update/*') || Request::is('admin/settings/appearance')) && !in_array('manage_settings', $get_role_data->permissions)){
        return false;
      }
      elseif((Request::is('admin/users/roles/list') || Request::is('admin/users/roles/add') || Request::is('admin/users/roles/update/*') || Request::is('admin/users/list') || Request::is('admin/user/add') || Request::is('admin/user/update/*') || Request::is('admin/user/profile')) && !in_array('manage_user', $get_role_data->permissions)){
        return false;
      }
  ...
  public static function placeholder_img_src(){
    return asset('resources/assets/images/no-image.png');
  }
  
  public static function avatar_img_src(){
    return asset('resources/assets/images/avatar.jpg');
  }
		
  public static function role_based_pricing_by_product_id($product_id){
    $pricing_data = array();
    $get_is_role_based_pricing_enable  =  PostExtra::where(['post_id' => $product_id, 'key_name' => '_is_role_based_pricing_enable'])->first();
    $get_role_based_pricing  =  PostExtra::where(['post_id' => $product_id, 'key_name' => '_role_based_pricing'])->first();
  ...
  public static function create_download_file_html( $post_id, $order_id ){
    $_this  =  new self;
    ...
    if( $_this->classCommonFunction->checkDownloadRequired( $post_id, $order_id ) ){
      $str = '';
      
      if(isset($get_product_data['post_type']) && $get_product_data['post_type']== 'product_variation'){
        if(count($get_product_data['_downloadable_product_data']) > 0){
          foreach($get_product_data['_downloadable_product_data'] as $key => $files){
            if(isset($files['uploaded_file_url']) && !empty($files['uploaded_file_url'])){
        ...
        if(count($get_product_data['_downloadable_product_files']) > 0){
          foreach($get_product_data['_downloadable_product_files'] as $key => $files){
            if(isset($files['uploaded_file_url']) && !empty($files['uploaded_file_url'])){
              $str .= '<div>'. Lang::get('frontend.downloadable_file_label') .': <a target="_blank" href="'. route('downloadable-product-download', [$post_id, $order_id, $key, 'uploaded_file_url']) .'">' .$files['file_name'] .'</a></div>';
            }
          }
