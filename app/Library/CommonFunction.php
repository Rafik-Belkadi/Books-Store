...
class CommonFunction
{
  ...
  * @return boolean
  */
  public function checkDownloadRequired($post_id, $order_id){
    $option   =  new OptionController();
    $product  =  new ProductsController();
    
    $get_settings  =  $option->getSettingsData();
    $get_product_data = $product->getProductDataById( $post_id );
    ...
    if(isset($get_product_data['post_type']) && $get_product_data['post_type']== 'product_variation'){
    if( ( ( !empty($get_product_data['_download_expiry']) &&  $get_product_data['_download_expiry'] <= $today ) || (empty($get_product_data['_download_expiry'])) ) && ((!empty($get_product_data['_downloadable_limit']) && count($count_download) < $get_product_data['_downloadable_limit']) || (empty($get_product_data['_downloadable_limit'])) ) ){
     
        $condition = true;
      }
    }
    else{
      if(( ( !empty($get_product_data['_downloadable_product_download_expiry']) &&  $get_product_data['_downloadable_product_download_expiry'] <= $today ) || (empty($get_product_data['_downloadable_product_download_expiry'])) ) && ((!empty($get_product_data['_downloadable_product_download_limit']) && count($count_download) < $get_product_data['_downloadable_product_download_limit']) || (empty($get_product_data['_downloadable_product_download_limit'])) )){
        $condition = true;
      }
    }
    
    if( (($get_settings['general_settings']['downloadable_products_options']['login_restriction'] == true && is_frontend_user_logged_in() && $get_settings['general_settings']['downloadable_products_options']['grant_access_from_thankyou_page'] == true) || ($get_settings['general_settings']['downloadable_products_options']['login_restriction'] == false && $get_settings['general_settings']['downloadable_products_options']['grant_access_from_thankyou_page'] == true)) && $condition === true){
      return true;
    }
    else{
      return false;
